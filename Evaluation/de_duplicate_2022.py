...
def read_visual_files(lst_dir):
    dic_formula_visual_id = {}
    for file in os.listdir(lst_dir):
        with open(lst_dir + "/" + file, mode='r', encoding="utf-8") as csv_file:
...
def deduplicate(dic_formula_visual_ids, submission_file_path, topic_visual_lst):
    ...
    for topic_id in tqdm(result_dict):
        if topic_id not in topic_visual_lst:
            continue
        
        temp_result_dic = result_dict[topic_id]
        sorted_items = sorted(temp_result_dic.items(), key=operator.itemgetter(1), reverse=True)
        
        visited_visual_ids = set()
        temp_dict = {}

        for formula_id, score in sorted_items:
            visual_id = dic_formula_visual_ids.get(formula_id)
            if visual_id is None or visual_id in visited_visual_ids:
                continue
            
            temp_dict[visual_id] = score
            visited_visual_ids.add(visual_id)
...
def deduplicated_prim_file(submission_dir, result_dir, file, topic_visual_lst, dic_formula_visual_ids):
    ...
    prim_dic = {}
    for topic_id in tqdm(topic_visual_lst):
        topic_prim_dic = {}
...
def main():
    ...
    dic_formula_visual_ids = read_visual_files(tsv_dir)
